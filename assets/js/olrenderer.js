...
function tranposeLine(point_ori, point_dest, distance) {
    ...
    var NewOri = [
        Math.sin(angle) * distance + startX, -Math.cos(angle) * distance + startY,
    ];
    var Newdest = [
        Math.sin(angle) * distance + endX, -Math.cos(angle) * distance + endY,
    ];
...
function transposePointVerticalyFromLine(point_ori, linePoints, distance) {
    ...
    var angle = Math.atan2(endY - startY, endX - startX);
    return [
        Math.sin(angle) * distance + point_ori[0], -Math.cos(angle) * distance + point_ori[1],
    ];
}
...
function orientedCurveArrow(style, ori, dest, rad_ori, rad_dest, width) {
    ...
    var extremPointArrow = [
        transposePointVerticalyFromLine(
            newDest, [newDest, center_curve_point],
            width / 2 + widthArrow * (width / 2)
        ),
        transposePointVerticalyFromLine(
            newDest, [newDest, center_curve_point], -(width / 2 + (widthArrow * width) / 2)
        ),
    ];

    newOri = [
        transposePointVerticalyFromLine(
            newOri, [newOri, center_curve_point],
            width / 2
        ),
        transposePointVerticalyFromLine(
            newOri, [newOri, center_curve_point], -width / 2
        ),
    ];
    newDest = [
        transposePointVerticalyFromLine(
        ...
        transposePointVerticalyFromLine(
            newDest, [newDest, center_curve_point], -width / 2
        ),
    ];
...
function orientedCurveOneArrow(style, ori, dest, rad_ori, rad_dest, width) {
    ...
    newOri = [
        ...
        transposePointVerticalyFromLine(
            newOri, [newOri, center_curve_point], -width
        ),
    ];
...
function noOrientedCurveArrow(style, ori, dest, rad_ori, rad_dest, width) {
    ...
    newOri = [
        transposePointVerticalyFromLine(
            newOri, [ori, center_curve_point],
            width / 2
        ),
        transposePointVerticalyFromLine(
            newOri, [ori, center_curve_point], -width / 2
        ),
    ];
    newDest = [
        transposePointVerticalyFromLine(
            newDest, [dest, center_curve_point],
            width / 2
        ),
        transposePointVerticalyFromLine(
            newDest, [dest, center_curve_point], -width / 2
        ),
    ];
